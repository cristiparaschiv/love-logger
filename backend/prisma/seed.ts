...
async function main() {
  ...
  // Seed daily check-in questions (skip if already seeded)
  const questionCount = await prisma.dailyQuestion.count();
  if (questionCount === 0) {
    ...
    await prisma.dailyQuestion.createMany({
      data: [
        ...freeTextQuestions.map((text) => ({ text, type: 'free_text', options: null as string | null })),
        ...optionsQuestions.map((q) => ({ text: q.text, type: 'options', options: JSON.stringify(q.options) })),
      ],
    });
    console.log('Seeded 100 daily check-in questions');
  } else {
    console.log(`Daily questions already seeded (${questionCount} found)`);
  }
